...
uint8_t file_name_convertor(char *file_name, const INDEX_ENTRY *index_entry) {
...
int read_clusters_to_buf(uint8_t **buf, uint64_t *buf_current_size, uint64_t *buf_size, int64_t lcn, uint64_t length,
...
int parse_data_run(uint64_t offset, GENERAL_INFORMATION *g_info, MAPPING_CHUNK **chunk) {
...
//int parse_data(uint64_t offset, GENERAL_INFORMATION *g_info, MAPPING_CHUNK **chunk) {
//    uint8_t length_byte;
//    uint8_t v;
//    uint8_t l;
//
...
//
//    do {
//        length_byte = *ptr_run_list;
//        v = length_byte & 0x0F;
//        l = length_byte >> 4;
//        uint64_t length = 0;
//        uint8_t count = v;
//
//        byte = (uint8_t *) ptr_run_list + v;
//        count = v;
//
//        result = 0LL;
//
//        while (count--) {
//            value = *byte--;
//            mask = value >> (byte_shift - 1);
//            result = (result << byte_shift) | ((value + mask) ^ mask);
//        }
//    }
//}
